...
CREATE DEFINER=`root`@`localhost` EVENT `config_event` ON SCHEDULE EVERY 1 MINUTE STARTS '2023-11-24 10:00:00' ENDS '2023-11-24 10:10:00' ON COMPLETION PRESERVE DISABLE DO CALL logs()$$

DROP EVENT IF EXISTS `generate`$$
CREATE DEFINER=`root`@`localhost` EVENT `generate` ON SCHEDULE AT '2023-11-24 10:00:00' ON COMPLETION PRESERVE DISABLE DO CALL generateTables()$$
